...
public class Sender {
        ...
        void generateRandom(){
            rando = (int)(Math.random() * (Math.pow(2,16) + 1)); //generate number between 1 and 2^16
            rando=(rando*1024)+1; //convert number to a multiple of (1024+1)
        }
        ...
        public void run() throws IOException, ClassNotFoundException {
            ...
            while ((slideWin <= Math.pow(2,16)+1) && SegNum<=100000){ //need to set this limit to 10Million
                    
                    System.out.println("---------------------------------");
                    sendFrameSize();

                    int loopCount=0;
                    while ((loopCount < slideWin) && SegNum<=100000) { //need to set this limit to 10Million
